...
export const AvatarManager: React.FC<AvatarManagerProps> = ({ token }) => {
  ...
  const fetchAvatars = async () => {
    try {
      const res = await fetch('https://coup-backend-lywm.onrender.com' + '/api/avatars');
      if (res.ok) {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const res = await fetch('https://coup-backend-lywm.onrender.com' + '/api/admin/upload/avatar', {
        method: 'POST',
  ...
  const handleDelete = async (id: string) => {
    try {
      const res = await fetch('https://coup-backend-lywm.onrender.com' + `/api/admin/avatars/${id}`, {
        method: 'DELETE',
        headers: { Authorization: `Bearer ${token}` },
      });
      if (res.ok) {
